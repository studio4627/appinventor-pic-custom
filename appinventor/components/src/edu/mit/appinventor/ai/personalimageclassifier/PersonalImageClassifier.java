...
public final class PersonalImageClassifier extends AndroidNonvisibleComponent implements Component {
  ...
  public void GotClassification(String result) {
    // The original parameter type is YailList
    EventDispatcher.dispatchEvent(this, "GotClassification", result.replace("(", "").replace(")",""));
  }
    ...
    public void reportResult(final String result) {
      ...
      try {
        ...
        form.runOnUiThread(new Runnable() {
          @Override
          public void run() {
            GotClassification(resultList.get(0).toString().replace("(", "").replace(")",""));
